...
object GTOMachineTooltipsA : AutoInitialize<GTOMachineTooltipsA>() {
    ...
    val DataFormTestingPlantMachineTooltips: ComponentListSupplier = ComponentListSupplier {
        ...
        command("该阶段时，需要根据测试等级向机器提供对应数量的任意AE物质" translatedTo "During this stage, you need to provide the machine with the corresponding amount of any AE material according to the test level")
        command("每次提供的AE物质将被机器消耗掉，并重置测试阶段的配方时间" translatedTo "Each provided AE material will be consumed by the machine and reset the recipe time of the testing stage")
        info("测试等级为n时，每次测试需要提供(2^n*2048)字节的AE物质数据" translatedTo "When the test level is n, each test requires (2^n*2048) bytes of AE material data")
        important("输入足够的AE物质后，测试等级将提升1级，并继续需求对应数量的AE物质，且不能与之前所提供的AE物质重复" translatedTo "After inputting enough AE material, the test level will increase by 1, and it will continue to require the corresponding amount of AE material, which cannot be the same as the previously provided AE material")
        guide("如需停止测试阶段，请暂停向机器提供AE物质，机器将自动进入分析阶段" translatedTo "To stop the testing stage, please pause providing AE material to the machine, and the machine will automatically enter the analysis stage")
        section("分析阶段" translatedTo "Analysis Stage")
        command("分析阶段时，机器会根据测试结果，转化(测试等级)²的数据存储研究点数，并存储于机器内的晶片中" translatedTo "During the analysis stage, the machine will convert (test level)² data storage research points based on the test results and store them in the data crystal inside the machine")
        command("分析阶段的耗时为45秒，期间与测试阶段消耗相同的功率" translatedTo "The analysis stage takes 45 seconds, consuming the same power as the testing stage")
    }
