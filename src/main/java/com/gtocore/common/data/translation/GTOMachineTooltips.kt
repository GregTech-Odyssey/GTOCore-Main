...
object GTOMachineTooltips {
    ...
    val DrawerStorageTooltips = ComponentListSupplier {
        ...
        info("升级插在主机自己的槽里：一格、最多 4 个（乘算，再多会溢出）" translatedTo "Upgrades go into the controller's own slot: a single stack of at most 4 (multiplicative, more would overflow)")
        ...
        info("流体的升级收益只有物品的一半（倍率 ÷2，至少 1）" translatedTo "Fluids only get half the upgrade benefit (multiplier / 2, at least 1)")
        info("创造（最大存储）升级不算倍率，也不收进主机槽" translatedTo "The creative (max storage) upgrade is not counted and cannot go into the controller slot")
