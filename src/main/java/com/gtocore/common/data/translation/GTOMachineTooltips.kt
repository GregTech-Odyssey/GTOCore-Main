...
object GTOMachineTooltips {
    ...
    val IndustrialPlatformDeploymentToolsTooltips = ComponentListSupplier {
        ...
        highlight("专业级工业基地搭建工具" translatedTo "Professional-grade industrial base construction tool") { gold() }
        highlight("泛银河格雷科技销售部核心产品" translatedTo "Core product of Pan-Galaxy Greg Technology Sales Department") { gold() }
        highlight("扩展包欢迎投稿你的蓝图" translatedTo "Expansion packs welcome submissions of your blueprints") { gold() }
    ...
    val PanGalaxyGregTechTradingStationTooltips = ComponentListSupplier {
        ...
        highlight("你的星际贸易帝国，从这里开始" translatedTo "Your interstellar trade empire starts here") { gold() }
        highlight("效率、规模与协作的终极贸易解决方案" translatedTo "The ultimate trading solution for efficiency, scale, and collaboration") { gold() }
    ...
    // ME无线连接机 — 妙妙工具置顶（勿用 NewDataAttributes 模板，避免 TooltipsSortedWrapper 重排到末尾）
    @JvmField
    val MeWirelessConnectionMachineTooltips = ComponentListSupplier {
    ...
    val DrawerStorageTooltips = ComponentListSupplier {
        ...
        info("主机加成：容量额外 ×（超级箱/缸等级 × 数量）" translatedTo "Controller bonus: capacity x (super chest/tank tier x count) extra")
        ...
        info("每种类容量 = 同类抽屉里最小的每槽容量（混放取小的）× 密封机械方块等级 × 均分后的升级倍率 × 主机加成" translatedTo "Capacity per type = the smallest amount per slot among the matching drawers (mixing picks the small one) x Hermetic Casing tier x the split upgrade multiplier x the controller bonus")
        info("升级倍率：所有升级的倍率相乘后按同类抽屉数量均分（乘积^(1/抽屉数)）——4 个抽屉 8 个升级就只乘 2 次，4 个抽屉 3 个升级乘 0.75 次" translatedTo "Upgrade multiplier: all upgrades multiplied together, then split over the matching drawer count (product^(1/drawers)) - 4 drawers with 8 upgrades multiplies twice, 4 drawers with 3 upgrades multiplies 0.75 times")
    ...
    val HeaterMachineTooltips = ComponentListSupplier {
        ...
        section(ComponentSlang.RunningRequirements)
        command("使用燃料加热，热量从顶面和侧面输出" translatedTo "Uses fuel to heat, heat outputs from the top and sides")
    ...
    val BlazeBlastFurnaceTooltips = ComponentListSupplier {
        ...
        section(ComponentSlang.RunningRequirements)
        command("需提供§b 2^( 电压等级 - 2 ) * 10mb/s§r（最低§b 10mb/s§r）的§6液态烈焰§r" translatedTo "Requires to provide §b 2^(Voltage tier - 2 ) * 10mb/s§r (at least §b10mb/s§r) of §6Liquid Blaze§r")
